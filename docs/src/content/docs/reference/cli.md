...
`doctor` checks `gh` and `az` availability. For GitLab PR and CI steps, install and authenticate `glab`. For Bitbucket Cloud PR and CI steps, install and authenticate `twg`. For Azure DevOps PR and CI steps, install the `azure-devops` extension and provide a PAT.
