...
- The provider CLI (`gh`, `glab`, `forgejo-axi`, or `twg`) is not installed for GitHub, GitLab, Forgejo, or Bitbucket Cloud
- The provider CLI is not authenticated for GitHub, GitLab, Forgejo, or Bitbucket Cloud
...
- Uses `gh` for GitHub, `glab` for GitLab, `forgejo-axi` for Forgejo, the `twg` CLI for Bitbucket Cloud, and `az` for Azure DevOps
