...
- The provider CLI (`gh`, `glab`, `forgejo-axi`, `twg`, or `tea`) is not installed for GitHub, GitLab, Forgejo, Bitbucket Cloud, or Gitea (GitHub also skips when `gh` is missing from `PATH`)
- The provider CLI is not authenticated for GitHub, GitLab, Forgejo, Bitbucket Cloud, or Gitea (GitHub reports a timed-out or interrupted `gh auth status` separately from auth failure; either still skips)
...
- Uses `gh` for GitHub, `glab` for GitLab, `forgejo-axi` for Forgejo, the `twg` CLI for Bitbucket Cloud, `tea` for Gitea, and `az` for Azure DevOps
