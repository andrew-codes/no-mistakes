...
// fakeTwgResponse mirrors internal/pipeline/steps' test-local type of the
// same name (see fakeTwg in helpers_test.go): the two are independent
// definitions in separate packages, connected only by the JSON shape they
// exchange through FAKE_TWG_RESPONSES, since a compiled non-test binary
// cannot import a _test.go type.
type fakeTwgResponse struct {
	Stdout   string
	ExitCode int
}

// twgMaskedFlags mirrors the test-side masking in internal/pipeline/steps'
// twgArgsKey so a canned response keyed on a masked argv still matches the
// real (unmasked) invocation this handler receives.
var twgMaskedFlags = map[string]bool{"--title": true, "--description": true}

// twgArgsKey joins argv into the same lookup key the test-side fakeTwg
// response table is keyed by.
...
// map (see fakeTwg in internal/pipeline/steps/helpers_test.go), matched by
// exact argv.
